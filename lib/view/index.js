...
CodeBiteView = React.createClass({
  ...
  renderMainButton: function() {
    return (
      <button ref="playButton" type="button" className={'codebite-play ' + (this.state.status === 'loading' ? 'loading' : '') } disabled={this.state.status === 'loading'} onClick={this.onClickPlayButton}>
      ...
      <div ref="wrapper">
        ...
        <div className={'codebite-result ' + resultsClassName}>
          ...
          <nav className="codebite-results-actions">
            ...
            <span className="codebite-clear" onClick={this.clearResults}>Clear output<i className="icon icon-clear"></i></span>
          </nav>
        </div>
      </div>
    );
