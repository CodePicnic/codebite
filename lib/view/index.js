...
class CodeBiteView extends preact.Component {
  ...
  componentDidMount() {
    ...
    instance.on('highlights:change', instance.applyHighlights);
  }
  ...
  addMark() {
    this.props.instance.highlights =  this.props.instance.editor.listSelections().map(function(selection) {
      return {
    ...
    this.props.instance.emit('highlights:change');
  }
  ...
  render(props, state) {
    ...
    return (
      <div>
        <nav className="codebite-status logo">
          <span>{props.name}</span>
          <span className="codebite-status-button" title="Highlight" onClick={this.addMark.bind(this)}><i className="icon icon-mark"></i></span>
        </nav>
